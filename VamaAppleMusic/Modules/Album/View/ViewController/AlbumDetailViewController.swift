...
final class AlbumDetailViewController: UIViewController, UIGestureRecognizerDelegate {
    
    // MARK: Properties
    
    private let albumDetailViewModel: AlbumDetailViewModel
    ...
    let artistNameLabel: UILabel = {
        ...
        label.textColor = UIColor(red: 142/255.0, green: 142/255.0, blue: 147/255.0, alpha: 1.0)
        return label
    }()
    ...
    let visitAlbumButton: UIButton = {
        ...
        button.setTitle(Constants.Title.visitTheAlbum, for: .normal)
    ...
    let genreContainerView: UIView = {
        ...
        view.layer.borderColor = UIColor(red: 0/255.0, green: 122/255.0, blue: 255/255.0, alpha: 1.0).cgColor
        return view
    }()
    ...
    init(_albumDetailViewModel: AlbumDetailViewModel) {
        albumDetailViewModel = _albumDetailViewModel
    ...
    func onClickAppStoreVisitBtn(_ sender : UIButton) {
        guard let url = URL(string: albumDetailViewModel.album.artistUrl ?? "") else { return }
...
extension AlbumDetailViewController {
    ...
    private func setData() {
        var imagePath = albumDetailViewModel.album.artworkUrl100 ?? ""
        
        // To show the high resolution image on detail screen if internet available
        // else show the cached image from the list screen
        if Reachability.isConnectedToNetwork() {
            imagePath = albumDetailViewModel.album.artworkUrl100?.replacingOccurrences(of: "100x100", with: "600x600") ?? ""
        }
        
        albumImageView.imageFromServerURL(urlString: imagePath)
        albumNameLabel.text = albumDetailViewModel.album.name
        artistNameLabel.text = albumDetailViewModel.album.artistName
        copyRightInfoLabel.text = albumDetailViewModel.copyrightInfo
        let releaseDate = getDate(dateString: albumDetailViewModel.album.releaseDate, thisFormat: "yyyy-MM-dd")
        let releaseDateString = toLocalStringWithFormat(date: releaseDate, dateFormat: "MMM dd, yyyy")
        releasedInfoLabel.text =  Constants.Title.released + releaseDateString
        genreLabel.text = albumDetailViewModel.album.genres.first?.name
        visitAlbumButton.addTarget(self, action: #selector(AlbumDetailViewController.onClickAppStoreVisitBtn(_:)), for: .touchUpInside)
    }
